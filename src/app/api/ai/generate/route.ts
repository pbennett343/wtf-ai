...
export async function POST(req: Request) {
    ...
    try {
        ...
        const imageInstruction = isBookIllustrations
            ? (referenceImage
                ? `Transform this image into a simplified, minimal black ink sketch illustration for a book. DYNAMICALLY DEPICT the specific action described: "${prompt}". STRICT TRADEMARK & TEXT RESTRICTIONS: Do NOT include any team logos, letters on caps, or text/wordmarks on jerseys (caps must be plain, jerseys unbranded with no team names or logos). Do NOT render any written text, dates, numbers, or digits anywhere in the image. Render ONLY clean black ink line art strokes and outlines of the subject and action. NO colors, NO shading, NO gray tones, NO background scenery. Isolated on a plain solid white background. MUST BE A PERFECT 1:1 SQUARE ASPECT RATIO.${prompt ? ` Context: ${prompt}` : ""}`
                : `Create a simplified, minimal black ink line art sketch illustration for a book that DYNAMICALLY DEPICTS the specific event and action described here: "${prompt}". Visual action details: "${enrichedContext}". STRICT TRADEMARK & TEXT RESTRICTIONS: Do NOT include any team logos, letters on caps, or text/wordmarks on jerseys (keep caps plain and jerseys completely unbranded without team names or logos). Absolutely NO written text, NO words, NO dates, NO numbers, and NO digits floating or written anywhere in the image. CRITICAL ACTION INSTRUCTION: Depict the key action vividly (e.g. if stat mentions being struck by lightning, show a dramatic black ink lightning bolt striking the pitcher; if it mentions a dunk, show the player dunking). Render ONLY clean black ink outlines and strokes. NO colors, NO shading, NO gray tones, NO background scenery. Isolated on a plain solid white background. MUST BE A PERFECT 1:1 SQUARE ASPECT RATIO.`)
            : (referenceImage
